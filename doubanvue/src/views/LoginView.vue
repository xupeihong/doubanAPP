<template>
    <div class="login-view">
        <h1>
      <a href="javascript:history.go(-1);">取消</a>登录豆瓣
    </h1>
        <form method="get" @submit.prevent="onSubmit()">
            <!-- <p v-if="error" class="tip error">用户名或密码有问题</p> -->
            <div class="form-user">
                <label>
                    <strong>邮箱</strong>
                    <input v-model="email" type="email" name="email" @input="updateData" placeholder="邮箱" class="email">
                </label>
            </div>
            <div class="form-pwd">
                <label>
                    <strong>请输入密码</strong>
                    <template v-if="passType === 'password'">
                        <input v-model="token" type="password" name="token" @input="updateData" placeholder="密码" class="pwd">
                    </template>
                    <template v-if="passType === 'text'">
                        <input v-model="token" type="text" name="token" @input="updateData" placeholder="密码" class="pwd">
                    </template>
                    <span class="show-pwd" :class="{show: isShow}" @click="showPwd()"></span>
                </label>
            </div>
            <div class="">
                <button class="submit" type="submit" :disabled="isDisabled" :class="{disabled: isDisabled}">
                    {{loginState}}
                </button>
            </div>
        </form>
        <div class="footer">
            <div class="more-login">使用其他方式登录 &amp; 找回密码</div>
            <div class="btns">
                <router-link :to="{name: 'RegisterView'}">注册豆瓣帐号</router-link>
                <a href="#">下载豆瓣App</a>
            </div>
        </div>
    </div>
</template>
<script>
import {
    mapState
} from 'vuex'
import $ from 'jquery'

export default {
    name: 'login-view',
    data() {
        return {
            loginState: '登录',
            isDisabled: false, // 提交按钮不能使用
            isShow: 0, // 不显示密码
            passType: 'password',
            error: '', // 验证失败返回信息
            type: false
        }
    },
    computed: {
        // 验证用户的账号和密码
        ...mapState({
            email: state => state.user.temp_email,
            token: state => state.user.temp_token

        })

    },
    methods: {
        showPwd: function() {
            this.isShow = this.isShow ? 0 : 1
            this.isShow ? this.passType = 'text' : this.passType = 'password'
        },
        // btnsub: function() {
        //     var email = $(".email").val();
        //     var pwd = $(".pwd").val();
        //     alert(1)
        //     $.ajax({
        //         url: 'http://127.0.0.1:3000/adduser/getuser',
        //         type: 'post',
        //         data: {
        //             name: email,
        //             pwd: pwd
        //         },
        //         success: function(data) {
        //             if (data.status == "ok") {
        //                 // alert(data.message);
        //                 alert(data.list[0].email);
        //                 alert(data.list[0].password);
        //                 if (data.list[0].email == email && data.list[0].password == pwd) {
        //                     // 可以登录
        //                     alert("可以");
        //                     this.type = true;
        //                     alert(this.type);

        //                 } else {
        //                     alert("不行");
        //                     // 不能登录，需要注册
        //                 }
        //             }
        //         },
        //         error: function(err) {
        //             console.log(err);
        //         }
        //     })
        // },
        updateData: function(e) {
            // 用户信息进行提交
            this.$store.commit({
                    type: 'updateData',
                    name: e.target.name,
                    value: e.target.value
                })
                // alert("fdsf");
        },
        beforeSubmit: function() {
            this.isDisabled = true
            this.loginState = '正在登录...'
        },
        onSuccess: function(res) {
            //登录成功跳转页面
            // alert("开始跳转");
            this.$router.push({
                name: 'StatusView'
            })
        },
        onError: function(err) {
            // this.error = err.body.error
            this.loginState = '登录'
            this.isDisabled = false
        },
        onSubmit: function() {
            // 登录中...
            this.beforeSubmit()
            alert("进来了");
            var email = $(".email").val();
            var pwd = $(".pwd").val();
            $.ajax({
                url: 'http://127.0.0.1:3000/adduser/getuser',
                type: 'post',
                data: {
                    name: email,
                    pwd: pwd
                },
                success: function(data) {
                    if (data.status == "ok") {
                        alert(data.list[0].email);
                        alert(data.list[0].password);
                        if (data.list[0].email == email && data.list[0].password == pwd) {
                            // 可以登录
                            // alert("可以1");
                            this.type = true;
                            // alert(this.type);
                            this.$router.push({
                                name: 'StatusView'
                            })
                        } else {
                            this.$router.push({
                                    name: 'LoginView'
                                })
                                // alert("不行");
                        }
                    }
                }.bind(this),
                error: function(err) {
                    console.log(err);
                }
            })

            // 登录成功跳转
            // this.$store.dispatch({
            //     type: 'login',
            //     email: this.email,
            //     token: this.token,

            // }).then(res => {
            //     // 成功返回结果
            //     this.onSuccess(res)
            //     alert("可以登录");
            // }, err => {
            //     // 失败返回错误
            //     alert("不能登录");
            //     // this.onError(err)
            // })
        }
    },
    // to : 将要进入的 路由对象
    // from : 当前导航正要离开的路由
    // next : 调用next()， resolve beforeEach这个钩子，即调用后，表示这个钩子函数结束了，同时里面可以设置一些跳转等
    beforeRouteEnter(to, from, next) {
        next(vm => {
            if (vm.$store.getters.currentUser.email) {
                // next({ path: '/' })
                // alert("准备跳转");
                vm.$router.push({
                    name: 'StatusView'
                })
            } else {
                next()
            }
        })
    },
    created() {
        // 得到本地账号自动填充
        if (localStorage.getItem('email')) {
            this.$store.commit({
                type: 'getLocalUser'
            })
        }
    }
}
</script>
<style scoped>
a {
    text-decoration: none;
}

.login-view {
    position: relative;
    top: -4rem;
}

.login-view h1 {
    height: 4.5rem;
    margin: 0 0 1rem 0;
    padding: 0 1.8rem;
    line-height: 4.5rem;
    background: #fff;
    border-bottom: 0.1rem solid #eee;
    text-align: center;
    font-size: 1.8rem;
    font-weight: bold;
}

.login-view h1 a {
    position: absolute;
    left: 1.8rem;
    top: 0;
    color: #42bd56;
    font-size: 1.5rem;
    font-weight: normal;
}

.login-view form {
    padding: 2rem 1.5rem;
}

.login-view form strong {
    font-size: 1.5rem;
    color: #222;
    display: none;
    margin-bottom: 0.5rem;
}

.login-view form input[type="email"],
input[type="text"],
input[type="password"] {
    display: inline-block;
    width: 100%;
    height: 4.4rem;
    padding: 0 0.8rem;
    box-sizing: border-box;
    font-size: 1.5rem;
    background: #fff;
    border: 0.1rem solid #ccc;
    border-top-left-radius: 0.3rem;
    border-top-right-radius: 0.3rem;
    outline: 0;
}

.login-view form .form-pwd {
    position: relative;
}

.login-view form .form-pwd input {
    padding-right: 4rem;
    border-top: 0;
}

.login-view form .form-pwd .show-pwd {
    position: absolute;
    right: 0.2rem;
    top: 0;
    display: block;
    height: 100%;
    width: 3.2rem;
    background-image: url(data:image/png;base64,);
    background-repeat: no-repeat;
    background-position: center center;
    background-size: 2.2rem;
    z-index: 3;
}

.login-view form .form-pwd .show-pwd .show {
    background-image: url(data:image/png;base64,);
}

.login-view form .submit {
    cursor: pointer;
    width: 100%;
    padding: 1.2rem 2.6rem;
    margin-top: 1rem;
    font-size: 1.7rem;
    text-align: center;
    color: #fff;
    background: #17AA52;
    border: 0.1rem solid #17AA52;
    border-radius: 0.3rem;
}

.login-view form .disabled {
    cursor: not-allowed;
    background: #eee;
    border: none;
}

.login-view form .tip {
    font-size: 1.4rem;
    color: #aaa;
}

.login-view form .error {
    color: #ff0000;
}

.login-view .footer .more-login {
    font-size: 1.5rem;
    color: #aaa;
    text-align: center;
}

.login-view .footer .btns {
    margin-top: 4rem;
    text-align: center;
    font-size: 1.5rem;
}

.login-view .footer .btns a {
    color: #42bd56;
    margin-right: 1.5rem;
}
</style>
