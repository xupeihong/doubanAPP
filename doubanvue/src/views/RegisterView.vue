<template>
    <div class="register-view">
        <template v-if="isComplete">
            <h1 class="title">注册成功</h1>
            <form method="post" onsubmit="return false">
                <!-- <p class="tip">请复制以下Token进行登录</p>
                <div class="form-alias">
                    <label>
                        <strong>token</strong>
                        <input v-model="token" type="text" name="token" placeholder="token" class="tok">
                    </label>
                </div> -->
                <div class="form-submit">
                    <router-link class="submit" to="/LoginView" tag="button">
                        去登录
                    </router-link>
                </div>
            </form>
        </template>
        <template v-else>
            <h1 class="title">欢迎加入豆瓣</h1>
            <form method="post" @submit.prevent="onSubmit()">
                <!-- <p v-if="error" class="tip error">{{warnmsg}}</p> -->
                <!-- <p>{{warnmsg}}</p> -->
                <div class="form-alias">
                    <label>
                        <strong>邮箱</strong>
                        <input v-model.trim="email" type="text" name="email" placeholder="邮箱" class="email" @blur="emailmsg()">
                    </label>
                    <span style="color:red; margin-left:13rem;">{{emailerr}}</span>
                </div>
                <div class="form-pwd">
                    <label>
                        <strong>请输入密码</strong>
                        <template v-if="passType === 'password'">
                            <input v-model.trim="pass" type="password" name="pass" placeholder="密码" class="pwd" @blur="pwdmsg()">
                        </template>
                        <template v-if="passType === 'text'">
                            <input v-model.trim="pass" type="text" name="pass" placeholder="密码" @blur="pwdmsg()" class="pwd">
                        </template>
                        <span class="show-pwd" :class="{show: isShow}" @click="showPwd()"></span>
                    </label>
                    <span style="color:red; margin-left:13rem;">{{pwderr}}</span>
                </div>
                <div class="form-name">
                    <label>
                        <strong>用户名</strong>
                        <input v-model.trim="name" type="text" name="name" placeholder="用户名" class="name" @blur="checkname" id="name">
                    </label>
                </div>
                <div class="form-submit">
                    <button class="submit " type="submit" :disabled="isDisabled" :class="{disabled: isDisabled}" @click="btnsubmit()">
                        {{registerState}}
                    </button>
                </div>
            </form>
            <div class="footer">
                <div class="agreement">点击「注册」代表你已阅读并同意用户使用协议</div>
                <div class="btns">
                    <a href="#">下载豆瓣App</a>
                </div>
            </div>
        </template>
    </div>
</template>
<script>
import $ from 'jquery'
import LoginView from './LoginView'

export default {
    name: 'register-view',
    data() {
        return {
            isComplete: false, // 注册已经完成
            isDisabled: false, // 提交按钮是否可用
            isShow: 0, // 不显示密码
            registerState: '立即注册',
            passType: 'password', // 密码隐藏
            error: '', // 返回错误验证失败
            email: '',
            pass: '',
            name: '',
            token: '',
            emailerr: '',
            pwderr: ''

        }
    },
    components: {
        LoginView
    },

    methods: {

        showPwd: function() {
            this.isShow = this.isShow ? 0 : 1
            this.isShow ? this.passType = 'text' : this.passType = 'password'
        },
        emailmsg: function() {
            var email = $(".email").val();
            var reg = /^\w{3,}@\w+(\.\w+)+$/;
            if (email == "") {
                this.emailerr = "邮箱不能为空"
            } else if (!reg.test(email)) {
                this.emailerr = "邮箱格式不对"
            } else {
                this.emailerr = ""
            }
        },
        btnsubmit: function() {
            var email = $(".email").val();
            var pwd = $(".pwd").val();
            var name = $(".name").val();
            if (name != "") {
                // 需要拿到token插入数据库中
                $.ajax({
                    url: 'http://127.0.0.1:3000/adduser',
                    type: 'post',
                    data: {
                        email: email,
                        password: pwd,
                        name: name
                    },
                    success: function(data) {
                        if (data.status == "ok") {
                            // alert(data.message);
                            $(".email").val();
                            $(".pwd").val();
                            $(".name").val();
                        }
                    },
                    error: function(err) {
                        console.log(err);
                    }
                })
            } else {
                alert("用户名不能为空");
            }

        },
        checkname: function() {
            var name = $("#name").val();
            // alert(name);
            console.log(name);
            $.ajax({
                url: 'http://127.0.0.1:3000/adduser/checknameS',
                type: 'POST',
                data: {
                    name: name
                },
                success: function(data) {
                    
                    if (data.status == 'ok') {
                        // alert(data.message);
                        // $("#name").css("color", "");
                        // $(".submit").attr("disabled", "false");
                        // window.location.reload();
                    } else if (data.status == "no") {
                        alert(data.message);
                        // $("#name").val("1");
                        // $("#name").css("color", "red");
                        $(".name").val();
                        // $(".submit").attr("disabled", "true");
                        // window.location.reload();
                        // this.registerState = '已经存在用户'
                    }

                    // if (data[0].username == name) {
                    //     // alert("此用户名已经存在");
                    //     alert("此用户名已经存在");
                    //      // $(".name").val("");
                    //   $("#name").text("");
                    //     // localStorage.removeItem(name);


                    // } else if (data[0].username != name) {
                    //     console.log("没问题");
                    //     // $(".submit").attr("disabled", false);
                    //     // name = $(".name").val();
                    // }
                },
                error: function(err) {
                    console.log(err);
                }
            })
        },
        pwdmsg: function() {
            var pwd = $(".pwd").val();
            var regs = /^[0-9A-Za-z]{6,}$/;
            // console.log(pwd);
            if (pwd == "") {
                this.pwderr = "密码不能为空"
            } else if (!regs.test(pwd)) {
                return this.pwderr = "密码长度不够6位"
            } else {
                this.pwderr = ""
            }

        },

        beforeSubmit: function() {
            this.isDisabled = true
            this.registerState = '正在提交...'
        },
        onSuccess: function(res) {
            // 注册后产生一个key
            this.isComplete = true
                // alert("注册成功");
            this.token = res.body.token
                // tol = res.body.token;

        },
        onError: function(err) {
            // this.error = err.body.error
            this.registerState = '立即注册'
            this.isDisabled = false
        },
        onSubmit: function() {
            // alert("近来了");
            // 提交按钮不能用
            // var name = $(".name").val();
            // if (name != "") {
            this.beforeSubmit();
            // }

            // 正在注册
            this.$store.dispatch({
                type: 'register',
                email: this.email,
                pass: this.pass,
                name: this.name

            }).then(res => {
                // 成功返回信息
                //同时往数据库插入用户信息
                this.onSuccess(res)
                    // console.log(res.body.email);
            }, err => {
                // 失败返回信息
                // this.onError(err)
            })
        }
    },
    beforeRouteEnter(to, from, next) {
        next(vm => {
            if (vm.$store.getters.currentUser.email) {
                // 跳转下个路由
                vm.$router.push({
                    name: 'HomeView'
                })
            } else {
                next()
            }
        })
    }
}
</script>
<style scoped>
.form-alias,
.form-pwd {
    margin-bottom: 25px;
}

.register-view h1 {
    margin: 10% 0 9%;
    font-size: 4rem;
    font-weight: 300;
    color: #42bd56;
    text-align: center;
}

.register-view form {
    padding: 2rem 1.5rem;
}

.register-view form strong {
    font-size: 1.5rem;
    color: #222;
    display: none;
    margin-bottom: 0.5rem;
}

.register-view form input::placeholder {
    color: #ccc;
}

.register-view form input[type="text"],
input[type="password"] {
    display: inline-block;
    width: 100%;
    height: 4.4rem;
    padding: 0 0.8rem;
    box-sizing: border-box;
    font-size: 1.5rem;
    background: #fff;
    border: 0.1rem solid #ccc;
    border-top-left-radius: 0.3rem;
    border-top-right-radius: 0.3rem;
    outline: 0;
}

.register-view form .form-pwd input,
.form-name input {
    padding-right: 4rem;
    border-top: 0;
}

.register-view form .form-pwd {
    position: relative;
}

.register-view form .form-pwd .show-pwd {
    position: absolute;
    right: 0.2rem;
    top: 0;
    display: block;
    height: 100%;
    width: 3.2rem;
    background-image: url(data:image/png;base64,);
    background-repeat: no-repeat;
    background-position: center center;
    background-size: 2.2rem;
    z-index: 3;
}

.register-view form .form-pwd .show-pwd .show {
    background-image: url(data:image/png;base64,);
}

.register-view form .submit {
    cursor: pointer;
    width: 100%;
    padding: 1.2rem 2.6rem;
    margin-top: 1rem;
    font-size: 1.7rem;
    text-align: center;
    color: #fff;
    background: #42bd56;
    border: 0.1rem solid #17AA52;
    border-radius: 0.3rem;
}

.register-view form .disabled {
    cursor: not-allowed;
    background: #eee;
    border: none;
}

.register-view form .tip {
    font-size: 1.4rem;
    color: #aaa;
}

.register-view form .error {
    color: #ff0000;
}

.register-view .footer .agreement {
    font-size: 1.4rem;
    color: #aaa;
    text-align: center;
}

.register-view .footer .btns {
    margin-top: 4rem;
    text-align: center;
    font-size: 1.5rem;
}

.register-view .footer .btns a {
    color: #42bd56;
    margin-right: 1.5rem;
    text-decoration: none;
}
</style>
